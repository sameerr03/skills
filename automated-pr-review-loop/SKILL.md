...
- Prefer an in-chat scheduled wake, recurring monitor, or another environment-native resumable wait. Do not use a terminal `sleep` command as the timer: it occupies a shell session, reduces responsiveness, and may require extra terminal polling to notice completion.
...
- If the environment requires a user-facing progress update before the next poll, report that the scheduled wait is still active without re-querying GitHub.
...
- Do not poll or post repeated review comments during the limited period. Wait until the reset time with an in-chat scheduled wake, recurring monitor, or another environment-native resumable wait, while continuing to provide progress updates without re-querying GitHub when the environment requires them.
